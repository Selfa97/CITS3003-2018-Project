...
const aiScene* scenes[numMeshes]; //Stores extra "scene" related information for each mesh
// -----Textures--------------------------------------------------------------
...
void loadMeshIfNotAlreadyLoaded(int meshNumber)
{
    ...
    //**********************
    //Get boneIDs and boneWeights for each verted from the imported mesh data
...
static void mousePassiveMotion(int x, int y)
{
    mouseX=x;
    mouseY=y;
    // SAM: Updates previous position to stop annoying jump when you click & move
    prevPos = currMouseXYscreen(x, y);
}
...
static void addObject(int id)
{
    ...
    //SAM AND BRAD: Changed angles[1] to = 0 so that the object faces the camera
    sceneObjs[nObjects].angles[0] = 0.0; sceneObjs[nObjects].angles[1] = 0.0;
...
//------BRAD: Function to delete an Object------------------------------------
//Deletes the current object, sets the mode to camera rotation, and redisplays
...
//-----BRAD: Function to restore a deleted  Object----------------------------
static void restoreDeletedObject(int objectId){
  if (nObjects >= maxObjects){
    return; //Max objects reached
  }

  //Sets most recent object to current object, and then sets the new current
  //Object to be the increment of nObjects, only works as reference isnt cleared
...
void drawMesh(SceneObject sceneObj)
{
    ...
    //BRAD: Modified model matrix to incude rotations
      mat4 model = Translate(sceneObj.loc) * RotateZ(sceneObj.angles[2]) *
      RotateY(sceneObj.angles[1]) * RotateX(sceneObj.angles[0]) * Scale(sceneObj.scale);
    ...
    //Set the new uniform shader variable BoneTransforms each time a mesh is drawn
    //********************
    int nBones = meshes[sceneObj.meshId]->mNumBones;
    if (nBones == 0){ //if no bones, use a single identity matrix
      nBones = 1;
    }
    //Get BoneTransforms for first (0th) animation at given time(in frames)
...
void display( void )
{
    ...
    // SAM: Add rotations to control camera [A]
    mat4 rotCam = RotateX(camRotUpAndOverDeg) * RotateY(camRotSidewaysDeg);
    view = Translate(0.0, 0.0, -viewDist) * rotCam;
...
static void lightMenu(int id)
{
    ...
    //Setting light menu interactions with light 2
    else if (id == 80){
      toolObj = 2;
      setToolCallbacks(adjustLocXZ,camRotZ(),
                       adjustBrightnessY, mat2(1.0, 0.0, 0.0, 10.0));
...
// SAM: Added in function callbacks for adjusting Object material [C]
static void adjustAmbientDiffuse(vec2 ad_xy)
{
    sceneObjs[currObject].ambient+=ad_xy[0];
    sceneObjs[currObject].diffuse+=ad_xy[1];
}

static void adjustSpecularShine(vec2 ss_xy)
{
    sceneObjs[currObject].specular+=ss_xy[0];
    sceneObjs[currObject].shine+=ss_xy[1];

    // SAM: Stop shine from exceeding 100
    if (sceneObjs[currObject].shine < -100)
    {
        sceneObjs[currObject].shine = -100;
    }
}
...
static void materialMenu(int id)
{
    ...
    // SAM: SET TOOL CALLBACKS FOR ADJUST OBJECT MATERIAL PROPERTIES [C]
    else if (id==20) {
        toolObj = currObject;
...
static void mainmenu(int id)
{
    ...
    //BRAD: Used to restore the last deleted object
    if (id == 98){
...
static void makeMenu()
{
    ...
    // SAM: CHANGED MENU TITLE TO SHOW IT HAS BEEN IMPLEMENTED [C]
...
void reshape( int width, int height )
{
    ...
    //BRAD: Drastically reduced nearDist Value from 0.2 to 0.01 to alow Objects
    //to be viewed closely without clipping
