...
void main()
{
 // Transform vertex position into eye coordinates
    ...
    vec3 L2 = normalize(LightPosition2.xyz);	  //Direction to second light source
    ...
    vec3 ambient = Light1colBrightness * AmbientProduct;
    vec3 ambient2 = Light2colBrightness * AmbientProduct;

    float Kd = max( dot(L, N), 0.0 );
    vec3  diffuse = Light1colBrightness * Kd * DiffuseProduct;

    //second lights diffuse product
    float Kd2 = max( dot(L2, N), 0.0);
    vec3 diffuse2 = Light2colBrightness * Kd2 * DiffuseProduct;

    float Ks = pow( max(dot(N, H), 0.0), Shininess );
    vec3  specular = Light1colBrightness * Ks * SpecularProduct;
    
    //second lights specular product
    float  Ks2 = pow( max(dot(N, H2), 0.0), Shininess );
    vec3 specular2 = Light2colBrightness * Ks2 * SpecularProduct;
    ...
    float fade = sqrt(dot(Lvec,Lvec))/5+1;

    color.rgb = ((ambient + diffuse)/fade) + globalAmbient  + ambient2 + diffuse2;
